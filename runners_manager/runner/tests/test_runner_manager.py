...
class TestRunnerManager(unittest.TestCase):
    ...
    def test_init_runner_manager(self):
        self.factory.create_runner.side_effect = [Runner('0', None, self.vm_type_normal), Runner('1', None, self.vm_type_normal)]
        r = RunnerManager(self.vm_type_normal, self.factory)

        self.assertEqual(self.factory.create_runner.call_count, 2)
    ...
    def test_update_runner(self):
        self.factory.create_runner.side_effect = [Runner('0', None, self.vm_type_normal), Runner('1', None, self.vm_type_normal), Runner('2', None, self.vm_type_normal)]
        r = RunnerManager(self.vm_type_normal, self.factory)
    ...
    def test_need_new_runner_current_full(self):
        self.factory.create_runner.side_effect = [Runner('0', None, self.vm_type_full), Runner('1', None, self.vm_type_full), Runner('2', None, self.vm_type_full), Runner('3', None, self.vm_type_full)]
        r = RunnerManager(self.vm_type_full, self.factory)
        self.assertEqual(r.need_new_runner(), False)
